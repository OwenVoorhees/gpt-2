...
model = GPT(GPTConfig(vocab_size=50304)) # Our model
model.to(device)
...
for step in range(max_steps):
    ...
    loss.backward()
    norm = torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0) # helps with unlucky in a batch from too big shocks of big gradients
    ...
    print(f"step {step}, loss : {loss.item()}, lr: {lr:.4e}, time: {dt:.2f} ms, tokens/sec: {tokens_per_sec:.2f}, grad norm: {norm:.4f}")
...
#tokens = enc.encode("Hello, you better run bud,")
